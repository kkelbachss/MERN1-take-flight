...
import { checkFlights, dateEditFormatter, getDataMsg } from '../../utils/helpers';
import { validatorSwitch, getErrorMsg, validateFlightTimes, validateAirport, validateFlightNum, validatePassengerCap } from '../../utils/validators';
...
function FlightSubmitForm() {
    ...
    async function submitHandler(e) {
        e.preventDefault();
        if (validateFlightTimes(formState.departureDate,formState.arrivalDate)
            &&validateAirport(formState.departureAirport)
            &&validateAirport(formState.arrivalAirport)
            &&validateFlightNum(formState.flightNumber)
            &&validatePassengerCap(formState.currentPassengerCount, formState.passengerCapacity)
            ) {
            //else post requests      
                ...
                if (res) {
                    ...
                    if (formState._id) {
                        try {
                            ...
                            let refresh = new Date().getTime() + (Math.floor(Math.random() * 1000));
    ...
    // i wanted this to warn the user of the requirements of each parameter but its acting up in react
        ...
        <Card style={{ margin: '5px'}}>
            <form id="flightSubmitForm" onSubmit={submitHandler}>
                ...
                <ListGroup>
                    ...
                    <div>
                        <input type="text" name="departureAirport" maxLength={3} value={formState.departureAirport} onChange={event => {setFormState({...formState, "departureAirport":event.target.value.toUpperCase()})}} onBlur={changeHandler}/>
                    </div>
                    <div className="form-row">     
                    ...
                    <ListGroupItem className="list-group-flush">
                        ...
                        <div>
                            <input type="text" name="arrivalAirport" maxLength={3} value={formState.arrivalAirport} onChange={event => {setFormState({...formState, "arrivalAirport":event.target.value.toUpperCase()})}} onBlur={changeHandler}/>
                        </div>
